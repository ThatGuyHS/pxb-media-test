...
// Re-enabled webhook secret for authentication - EYS esport specific
const WEBHOOK_SECRET = process.env.ESPLAY_WEBHOOK_SECRET_EYS || 'armless-dandelion-undercoat';
...
const PRIVATE_KEY = process.env.ESPLAY_PRIVATE_KEY_EYS || '';

// EBAS API configuration - Updated for EYS esport
const EBAS_API_URL = 'https://ebas.esportunited.com/apis/submit_member_with_lookup.json';
const EBAS_API_KEY = 'l4Hcaf4+1ilWa6jc1FW/Y/F74pmDlo7L';
...
async function waitForAvailableSlot(): Promise<void> {
  while (activeRequests >= MAX_CONCURRENT_REQUESTS) {
    console.log(`🚦 [EYS] Rate limit: ${activeRequests}/${MAX_CONCURRENT_REQUESTS} active requests, waiting...`);
...
async function submitToEbas(ebasData: any): Promise<{ success: boolean, response: any }> {
  try {
    console.log('🔄 [EYS] Submitting to EBAS API...');
    console.log('📤 [EYS] EBAS Payload:', JSON.stringify(ebasData, null, 2));
    ...
    console.log('📥 [EYS] EBAS Response:', JSON.stringify(responseData, null, 2));
    ...
    console.error('❌ [EYS] EBAS API Error:', error);
    return {
      success: false,
...
export async function POST(request: NextRequest) {
  console.log('📥 [EYS] Esplay webhook received:', new Date().toISOString());

  try {
    return await processWebhookRequest(request);
  } catch (error) {
    console.error('❌ [EYS] Webhook processing failed:', error);
...
async function processWebhookRequest(request: NextRequest): Promise<NextResponse> {
  ...
  if (WEBHOOK_SECRET) {
    console.log('🔐 [EYS] Checking webhook secret authentication');
    const authHeader = request.headers.get('Authorization');
    if (authHeader !== WEBHOOK_SECRET) {
      console.error('❌ [EYS] Authentication failed: Invalid webhook secret');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    console.log('✅ [EYS] Authentication successful');
  } else {
    console.log('⚠️ [EYS] No webhook secret configured, skipping authentication');
  }
  ...
  console.log(`📄 [EYS] Content-Type: ${contentType}`);

  if (contentType === 'text/plain' && PRIVATE_KEY) {
    console.log('🔒 [EYS] Encrypted payload detected, attempting to decrypt');
    ...
    try {
      ...
      console.log('🔓 [EYS] Successfully decrypted payload');
    } catch (error) {
      console.error('❌ [EYS] Decryption error:', error);
      return NextResponse.json({ error: 'Failed to decrypt payload' }, { status: 400 });
    }
  } else {
    console.log('📝 [EYS] Unencrypted JSON payload detected');
    // Handle regular JSON payload
    payload = await request.json();
  }

  console.log('📋 [EYS] Received payload:', JSON.stringify(payload, null, 2));

  // Validate the payload
  if (!payload || !payload.event || !payload.ssn) {
    console.error('❌ [EYS] Invalid payload: Missing required fields');
    return NextResponse.json({ error: 'Invalid payload' }, { status: 400 });
  }

  console.log(`✨ [EYS] Processing event: ${payload.event}`);
  ...
  if (REQUEST_QUEUE.has(requestKey)) {
    console.log(`🔄 [EYS] Request already in progress for key: ${requestKey}, waiting for result...`);
    try {
      const result = await REQUEST_QUEUE.get(requestKey);
      console.log(`✅ [EYS] Using cached result for duplicate request`);
      return result;
    } catch (error) {
      console.log(`❌ [EYS] Cached request failed, processing new request`);
  ...
  if (hasProcessedEntry(payload.ssn)) {
    console.log(`⚠️ [EYS] Duplicate entry received for SSN: ${payload.ssn}`);
  ...
  activeRequests++;
  console.log(`📊 [EYS] Active requests: ${activeRequests}/${MAX_CONCURRENT_REQUESTS}`);
  ...
  try {
    ...
    console.log('✅ [EYS] Webhook processed successfully');
    return result;
  } finally {
    // Always clean up
    activeRequests--;
    REQUEST_QUEUE.delete(requestKey);
    console.log(`📊 [EYS] Request completed. Active requests: ${activeRequests}/${MAX_CONCURRENT_REQUESTS}`);
  }
}

async function processUserData(payload: any): Promise<NextResponse> {
  // Process the webhook data
  console.log(`✅ [EYS] New user joined: ${payload.username} (${payload.first_name} ${payload.last_name})`);
  console.log(`📧 [EYS] Email: ${payload.email}, Phone: ${payload.phone}`);
  console.log(`🎮 [EYS] Steam ID: ${payload.steamid || 'Not provided'}`);
  console.log(`🕒 [EYS] Joined at: ${new Date(payload.joined_association * 1000).toISOString()}`);

  // Always store the SSN first to prevent duplicate processing
  addProcessedEntry(payload.ssn, payload);
  console.log(`📝 [EYS] Added SSN to processed entries database`);
  ...
  if (!ebasResult.success) {
    console.log(`❌ [EYS] EBAS API failed, returning error response`);
...
export async function GET() {
  console.log('⚠️ [EYS] GET request received, method not allowed');
